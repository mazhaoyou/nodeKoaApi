...
async function indexAction(ctx) {
  ...
  //获取所有的分类
  const data = await mysql("shop_category").where({
    "parent_id": 0
  }).select();
  const currentCategory = [];
  if (categoryId) {
    //获取分类里的子类
    currentCategory = await mysql("shop_category").where({
...
async function currentAction(ctx) {
  ...
  const data = {};
  //获取分类里的子类
  const currentOne = await mysql("shop_category").where({
    "id": categoryId
  }).select();
  const subList = await mysql("shop_category").where({
...
async function categoryNav(ctx) {
  ...
  //获得他的同类
  const navData = await mysql("shop_category").where({
